---
type: player
fpl_id: 448
player: Burn
team: NEW
position: DEF
current_rank: 284
segment: Undrafted buffer
tier: D
last_reviewed: 2026-08-08T16:50:00+10:00
---

# Burn

## Current assessment

- Rank: **162**
- Segment / tier: **Undrafted buffer / D**
- Availability: Available
- Review: [[06 Reviews/2026/08/2026-08-02/1148-AEST-review]]

## Pairwise placement

- Ranks below [[02 Players/Georginio - 125|Georginio]] after expected-points, minutes and risk comparison.
- Ranks above [[02 Players/Wieffer - 130|Wieffer]] after expected-points, minutes and draft replacement-value comparison.

## Confidence and reversal trigger

Confidence is medium-low because ranks 141 onward are still being manually stabilised. Revisit for confirmed starting role, repeated probable-first-team minutes, set-piece responsibility, injury recovery, suspension or transfer-driven competition.

## Pairwise review — 1602-AEST

- Current rank: **162**
- Segment/tier: **Undrafted buffer / D**
- API status: Available
- Outcome: retained after comparison with immediate neighbours and plausible challengers in ranks 136–175.
- Evidence: [[06 Reviews/2026/08/2026-08-02/1602-AEST-review]]
- Reversal trigger: confirmed role, transfer, injury, set-piece or repeated probable-first-team preseason evidence.

<!-- 0741-aest-newcastle-team-review -->
## Newcastle team comparison — 0741-AEST

- Internal Newcastle rank: **16 of 19**.
- Overall rank: **280** (was 162).
- Segment/tier: **Undrafted buffer / D**.
- Comparator outcome: minutes floor but low attacking ceiling.
- Reversal trigger: verified change in minutes, role, set pieces, penalties, fitness, transfer status or first-choice position.
- Evidence: [[06 Reviews/2026/08/2026-08-03/0741-AEST-review]].


## 0854-AEST defender positional comparison

- Defender order: **95** after block 3 with challengers 56–60 and 91–95.
- Overall rank: **280 → 287**.
- Raw expected points were compared before minutes, attacking role, clean-sheet environment, set pieces, injury/rotation risk, floor and ceiling.
- Evidence and reversal triggers: [[06 Reviews/2026/08/2026-08-04/0854-AEST-review]].


## 0920-AEST final defender positional comparison

- Defender order: **89** after the final block with challengers 86–90.
- Overall rank: **287 → 270**.
- Raw expected points were compared before minutes, attacking role, clean-sheet environment, set pieces, injury/rotation risk, floor and ceiling.
- Evidence and reversal triggers: [[06 Reviews/2026/08/2026-08-04/0920-AEST-review]].


## 0806-AEST cross-position comparison

- Reviewed with overall ranks 241–270 and challengers 236–240 and 271–275.
- Overall rank: **270 → 264**.
- Raw expected season points came first, then minutes, role, set pieces, floor/ceiling and positional replacement value.
- Draft-horizon injury weighting applied; short absences were minor, unknown or multi-month cases material.
- Evidence and reversal triggers: [[06 Reviews/2026/08/2026-08-05/0806-AEST-review]].


## 1650-AEST final full-board sweep

- Overall rank: **264 -> 284**.
- Current status: **Available**.
- Reconciled against the current FPL API and season-long Draft injury/transfer framework.
- Evidence and reversal triggers: [[06 Reviews/2026/08/2026-08-08/1650-AEST-review]].
