...
che=[(i,p) for i,p in rows if p[3]=='CHE']; assert len(che)==25,len(che)
...
comparisons='\n'.join(f"- **{ORDER[i]} over {ORDER[i+1]}** — {REASONS[ORDER[i]]}; confidence {'medium' if i<15 else 'low'}. Reverse with verified role, fitness, set-piece or minutes evidence." for i in range(len(ORDER)-1))
review.write_text(f"---\ntype: review\nreviewed_at: {TS}\nteam: CHE\n---\n\n# Chelsea internal FPL Draft review — {STAMP}\n\n## Scope\nAll 25 currently ranked Chelsea players were compared directly. Raw expected points were assessed first, then minutes, role, set pieces, injury and rotation risk, floor and ceiling; positional replacement value was applied afterward.\n\n## Evidence\n- Official FPL bootstrap API for identity, team, position and availability.\n- Current canonical board and Chelsea team note.\n- Welbeck was excluded because the latest canonical board assigns him to Brighton.\n\n## Decisive comparison chain\n{comparisons}\n\n## Final internal order\n"+'\n'.join(f'{i+1}. {n} — overall {r}' for i,(n,r) in enumerate(zip(ORDER,slots)))+'\n\n## Uncertainties\nThe forward and winger hierarchy is highly unstable. James, Fofana and Emegha remain fitness-sensitive.\n')
