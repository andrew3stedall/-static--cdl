...
reasons={
'B.Fernandes':'penalties, set pieces and the strongest secure raw-points projection',
'Mbeumo':'elite direct goal involvement and strong minutes outlook',
'Cunha':'central attacking role and high goal-assist ceiling',
'Šeško':'forward scarcity and central striker ceiling, discounted for fitness',
'Amad':'advanced role and chance creation with some rotation risk',
'Dorgu':'attacking wing-back upside and positional scarcity',
'Rashford':'proven scoring ceiling, heavily discounted for role uncertainty',
'Lammens':'starting-goalkeeper floor, discounted for replaceability',
'Maguire':'secure defensive minutes and aerial threat',
'Shaw':'attacking full-back upside with durability risk',
'Dalot':'minutes and attacking contribution, but role competition remains',
'Martinez':'passing and clean-sheet floor, discounted for injury',
'Mount':'attacking midfield ceiling but weak minutes certainty',
'Mainoo':'minutes and progression floor with limited direct FPL output',
'Zirkzee':'forward scarcity but unclear hierarchy and minutes',
'Yoro':'long-term centre-back ceiling with modest immediate attacking value',
'Mazraoui':'versatility and possible attacking role, but uncertain starts',
'Heaven':'development upside with low first-choice certainty',
'De Ligt':'centre-back floor heavily discounted by back injury',
'Tielemans':'current API assignment retained, but injury and role uncertainty dominate',
'Darlow':'backup-goalkeeper profile and current injury concern',
'Andrey Santos':'lowest current Manchester United role certainty in the canonical pool',
}
...
for i,l in enumerate(lines):
    if re.match(r'^\|\s*\d+\s*\|',l):
        ...
        if len(parts)>=10:
            rows.append((i,parts))
mun=[(i,p) for i,p in rows if p[3]=='MUN']
assert len(mun)==22, len(mun)
slots=sorted(int(p[0]) for _,p in mun)
byname={p[1]:(i,p) for i,p in mun}
assert set(order)==set(byname), (set(order)-set(byname),set(byname)-set(order))
old={n:int(byname[n][1][0]) for n in order}
# destination slot metadata controls segment/tier
...
for n,rank in zip(order,slots):
    ...
    entries.append(f'{rank}. [[{rel}|{n}]] — {p[2]}, MUN; {seg} / {tier}; {status}')
...
comparisons=[]
for a,b in zip(order,order[1:]): comparisons.append(f'- **{a} over {b}** — {reasons[a]}; confidence {"medium" if order.index(a)<12 else "low"}. Reverse with verified role, fitness, set-piece, transfer or minutes evidence.')
...
for p in changed:
    ...
    rows.append(f'| {TS} | `{p.as_posix()}` | {action} | Manchester United internal team ordering review | [[06 Reviews/2026/08/2026-08-03/{STAMP}-review]] | https://fantasy.premierleague.com/api/bootstrap-static/ |')
cl.write_text(base+'\n'+'\n'.join(rows)+'\n')
